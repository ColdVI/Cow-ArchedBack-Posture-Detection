...
from cowarch.detector_training import evaluate_detector_topline, validate_detector_dataset
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Held-out image/mask/withers_x/sacrum_x metadata CSV.",
    )
    ...
    topline, details = evaluate_detector_topline(evaluation_model, args.topline_eval)
    atomic_write_csv(details, args.output_dir / "topline_evaluation.csv")
    payload = {
        **coverage,
        **topline,
        "model": args.model,
