...
def ensure_annotation_columns(frame: pd.DataFrame) -> pd.DataFrame:
    """Add the two-pass columns and interpret legacy ``reviewed_by`` values.

    The migration is intentionally non-destructive. A legacy reviewer is copied
    into ``posture_reviewed_by`` for labeled rows and into
    ``geometry_reviewed_by`` when legacy geometry is present. The original
    ``reviewed_by`` column remains available to older readers.
    """
